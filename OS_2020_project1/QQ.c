...
int queue_pop(int* start, int* end){
	int front = -1;
	if(*start != *end){
		front = queue[*start];
		*start = ((*start) + 1) % MM;
...
void queue_push(int k, int* start, int* end){
	queue[(*end) % MM] = k;
	*end = ((*end) + 1) % MM;
	return;
}
...
void scheduler(Process* job,int num_jobs,char* method){
	...
	/*Use a queue*/
	if(method[0] == 'R')
		int* queue = (int*) malloc(MM * sizeof(int));
	...
	while(1){
		...
		switch(c){
			...
			case 'R':
				...
				next = RR(job, now_running, &start, &end, time_slice);
				time_slice--;
				break;
			default:
